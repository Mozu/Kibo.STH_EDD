...
async function route(context, callback, getEasyPostClientFn) {
  ...
  console.log(`DEBUGss method: ${method}, request context: `, requestContext);
  //console.log(`DEBUG requestPayload: `,requestPayload);
  if (method === 'transit-times') {
    return await getTransitTimes(context, requestPayload, getEasyPostClientFn);
  }
 ...
 */
async function getTransitTimes(context, request, getEasyPostClientFn) {
  ...
  const client = getEasyPostClientFn(context);
